...
const Admin = () => {
  ...
  const handleSample = async () => {
    ...
    setError('');
    const data = [
      { name: '시청 대강당', address: '서울시청 1층', description: '대규모 회의 및 행사 가능', imageUrl: '' },
      { name: '구민 체육관', address: '서울구로구 구로동 123', description: '실내 체육시설', imageUrl: '' },
      { name: '동주민센터 회의실', address: '서울강남구 역삼동 456', description: '소규모 모임/회의', imageUrl: '' },
      { name: '공공도서관 세미나실', address: '서울서초구 서초동 789', description: '세미나 및 강연', imageUrl: '' },
      { name: '청소년 문화의집', address: '서울마포구 합정동 101', description: '청소년 전용 문화공간', imageUrl: '' }
    ];
    try {
      for (const f of data) {
              ...
              <div key={facility.id} className="rounded-2xl bg-white border border-blue-100 shadow-md flex flex-col overflow-hidden hover:shadow-lg transition relative">
                ...
                <div className="p-4 flex-1 flex flex-col">
                  ...
                  <div className="flex gap-2 mt-auto">
                    <button className="text-blue-500 font-semibold hover:underline text-sm" disabled>수정</button>
                    <button className="text-red-500 font-semibold hover:underline text-sm" onClick={async () => {
